...
def fetch_emails(mail: imaplib.IMAP4_SSL, config: dict) -> List[Dict[str, Any]]:
    ...
    if time_window and fetch_unread:
        since_date = (datetime.now() - timedelta(minutes=time_window)).strftime(
            "%d-%b-%Y"
        )
        ...
        search_criteria = (
            "ALL"
            if not time_window
            else f'SINCE {(datetime.now() - timedelta(minutes=time_window)).strftime("%d-%b-%Y")}'
        )
    ...
    for email_id in email_ids:
        status, msg_data = mail.fetch(
            email_id, "(RFC822)"
        )  #  padrão para mensagens de e-mail (incluindo cabeçalhos e corpo).
        if status != "OK":
            continue
        ...
        matches_subject = (
            any(keyword.lower() in subject.lower() for keyword in subject_keywords)
            if subject_keywords
            else True
        )
        matches_sender = (
            any(domain.lower() in sender.lower() for domain in sender_domains)
            if sender_domains
            else True
        )
...
def parse_email_content(email_data: Dict[str, Any], config: dict) -> Dict[str, Any]:
    ...
    for bank, rules in parsing_rules.items():
        if any(
            domain in sender
            for domain in config.get("filtering", {}).get("sender_domains", [])
        ):
            bank_key = bank
            break
    if not bank_key:
    ...
    for part in email_message.walk():
        ...
        if content_type == "application/octet-stream":
            ...
            try:
                ...
                print(e)
                print(
                    f"Skipping unreadable octet-stream attachment for Email ID: {email_data['id']}"
                )
